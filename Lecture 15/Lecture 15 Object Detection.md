...
3번 과정에서, 파이프라인Threshold 이상이면 출력, 이하면 무시하는 방법Convnet의 Weight는 모두 동일해야한다. 혹은 score이 높은 K개의 proposal만 남기는 방법이 있다.
...
여기서 AP = 1 인 상황이 나오려면 모든 ground truth 박스에 대해서 모두 IoU > 0.5이어야 하며, true positive보다 false positive가 빠르게 정렬되면 안된다. 사실상 false positive이 없어야한다. 그리고 duplicate detection이 없어야한다..
...
즉, RoI는 변하더라도, 그 output은 변하지 않는다는 것이다!

=

CNN에 넣어서 계산할 수 있다. Backpropagation을 할 수 있다!
...
trick을 써서 box Transform도 내놓도록한다.
...
1. raw image와 feature map을 더 많이 접하기 때문
